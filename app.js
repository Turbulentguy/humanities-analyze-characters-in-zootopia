...
const fallbackKeywordsBySentiment = {
...
const isMediumViewport = () => window.matchMedia("(max-width: 860px)").matches;
...
const fallbackColorByIndex = (index) => fallbackPalette[index % fallbackPalette.length];

const getTotalDialogues = () => characters.reduce((sum, character) => sum + character.dialogues, 0);

const getSortedCharacters = () => [...characters].sort((a, b) => b.dialogues - a.dialogues);

const getFeaturedCharacters = () => getSortedCharacters().slice(0, 5);

const getTopFivePlusOthers = () => {
...
const normalizeSentimentCategory = (score, category) => {
...
const normalizeCharacterData = (name, dialogues, score, category, index) => {
...
const loadCharactersFromCsv = async () => {
...
const sentimentFillStyle = (sentiment) => {
  if (sentiment === "Negative") {
...
const chartFillColor = (character) => {
...
const percentageFor = (value, values) => {
...
const formatSignedSentiment = (score) => `${score >= 0 ? "+" : ""}${score.toFixed(SENTIMENT_DECIMALS)}`;

const profileUrlFor = (name) => `character.html?name=${encodeURIComponent(name)}`;
...
const customLabelPlugin = {
  ...
  afterDatasetsDraw(chart) {
    ...
    ctx.save();
    ctx.font = compactViewport ? "700 9px Sora" : isMediumViewport() ? "600 10px Sora" : "600 11px Sora";
    ...
    chart.getDatasetMeta(0).data.forEach((bar, index) => {
      ...
      if (state.mode === "sentiment") {
        ...
        const y = value >= 0 ? bar.y - (compactViewport ? 5 : 8) : bar.base + (compactViewport ? 13 : 16);
      ...
      ctx.fillText(label, bar.x, bar.y - (compactViewport ? 5 : 8));
    });
...
const renderChart = () => {
  const canvas = document.getElementById("dialogueChart");
  ...
  const compactViewport = isCompactViewport();
  const mediumViewport = isMediumViewport();
  ...
  state.chart = new chartConstructor(canvas, {
      ...
      datasets: [
        {
          ...
          borderRadius: 12,
          borderSkipped: false,
          maxBarThickness: compactViewport ? 34 : mediumViewport ? 44 : 58
        }
      ]
    },
    ...
    options: {
      ...
      layout: {
        padding: {
          top: compactViewport ? 18 : 12,
          right: compactViewport ? 14 : 10,
          bottom: compactViewport ? 26 : 10,
          bottom: compactViewport ? 54 : 10,
          left: compactViewport ? 8 : 6
        }
      },
      plugins: {
      ...
      scales: {
        x: {
          ...
          ticks: {
            color: "#334e72",
            autoSkip: false,
            maxRotation: compactViewport ? 58 : 0,
            minRotation: compactViewport ? 58 : 0,
            padding: compactViewport ? 12 : 8,
            font: { family: "Source Sans 3", size: compactViewport ? 9 : 12, weight: 600 },
        ...
        y: {
          ...
          ticks: {
            color: "#57708f",
            font: { family: "Source Sans 3", size: compactViewport ? 10 : 12, weight: 600 },
            callback: (value) =>
              state.mode === "sentiment"
